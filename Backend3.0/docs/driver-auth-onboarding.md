...
| `dob` | ✅ | Date of birth, `YYYY-MM-DD` (e.g. `1990-04-12`) |
| `email` | ❌ | Lowercased; unique if provided |
| `gender` | ❌ | `male` \| `female` \| `other` (case-insensitive) — render as a picker, see below |
| `address` | ❌ | Free text |
...
**jpeg / jpg / png / webp only, max 5 MB each.**

| Field | Required | Max files | Purpose |
| ----- | -------- | --------- | ------- |
...
## Form validation (validate before "Next")

The registration form is multi-step. **Block the "Next" button on each step
until that step's fields pass local validation** — don't wait for the server to
reject the final submit. The backend enforces the same rules and will return
`400` with an `errors[]` array, but the driver shouldn't get that far.

### Step 4a — Personal details

| Field | Control | Rule to enforce before "Next" |
| ----- | ------- | ----------------------------- |
| `profileImage` | Photo picker (camera / gallery) | **Mandatory.** A file must be selected; type ∈ jpeg/jpg/png/webp; size ≤ 5 MB. Show the preview thumbnail once picked |
| `name` | Text | **Mandatory.** Non-empty after trim |
| `dob` | **Date picker** (not free text) | **Mandatory.** Must be a real date, sent as `YYYY-MM-DD`. Cap the picker's max date so a future date can't be chosen |
| `gender` | **Dropdown / segmented box** | Optional, but if the box is shown it must contain exactly **Male**, **Female**, **Other** — no free text. Send the lowercase value (`male` / `female` / `other`) |
| `email` | Text | Optional. If filled, must look like an email |
| `address` | Text | Optional |

> `profileImage` and `dob` are **hard requirements** — the server rejects the
> submit without them:
> ```json
> {
...
>     { "field": "profileImage", "message": "Profile photo is required" }
>   ]
> }
> ```
> An unparseable date comes back separately as
> `{ "message": "Date of birth is invalid", "errors": [{ "field": "dob", ... }] }`.

### Gender box

Bind the three options to these exact values — anything else is a `400`
...
Matching is case-insensitive server-side, so `Male` also works, but send
lowercase for consistency. Leave the field out entirely if the driver skips it —
don't send an empty string.

### Step 4b — Documents & vehicle

| Field | Rule |
| ----- | ---- |
| `vehicleTypeId` | **Mandatory.** One option selected from step 3 |
| `vehicleNumber` | **Mandatory.** Non-empty; uppercase it client-side |
| `dlNumber`, `aadharCardNumber` | Optional; if entered, validate format locally |
| All image fields | Optional; enforce the same type/size limits as the profile photo |
...
| ------ | ---- | -------- |
| `400` | `{ "message": "All required fields must be provided", "errors": [{ "field": "vehicleNumber", "message": "Vehicle number is required" }] }` | Highlight fields from `errors[]` — covers `name`, `phoneNumber`, `dob`, `profileImage`, `vehicleTypeId`, `vehicleNumber` |
| `400` | `{ "message": "Date of birth is invalid", "errors": [{ "field": "dob", ... }] }` | Send `YYYY-MM-DD` from the date picker |
...
form.append('dob', dob);                        // required — 'YYYY-MM-DD'
form.append('profileImage', profileImage);      // required — File/Blob
form.append('vehicleTypeId', selectedType._id);
form.append('vehicleNumber', vehicleNumber.toUpperCase());
if (email) form.append('email', email);
if (gender) form.append('gender', gender);      // 'male' | 'female' | 'other'
