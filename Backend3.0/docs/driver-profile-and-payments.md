...
>
> A driver can have **exactly one** vehicle. It's created during onboarding and
> can only be edited afterwards — never added to. This is enforced by a unique
> index on the database, not just a check, so there is no way around it.
>
> **Build the UI as a single "My Vehicle" screen with an Edit action.** No list,
> no "Add vehicle" button, no delete.

### 2a. Read the driver's vehicle

**`GET /api/v3/vehicles/my`** — Bearer token

Returns a list for historical reasons, but it will contain **at most one**
entry — read `data[0]` and treat an empty `data` as "no vehicle yet".

`vehicleTypeId` comes back **populated** with the full type object, so you can
render the name and icon directly.
...
```js
const { data } = await fetch(`${BASE_URL}/vehicles/my`, {
  headers: { Authorization: `Bearer ${token}` },
}).then((r) => r.json());

const vehicle = data[0] ?? null;   // at most one
```
...
### 2c. `POST /api/v3/vehicles` — recovery only

The vehicle is created during onboarding, so the app should **not** expose an
"Add vehicle" flow. This endpoint exists only for the edge case where a driver
somehow has no vehicle (`GET /vehicles/my` returned an empty `data`).

Multipart, Bearer token, same fields as the edit call, with `vehicleTypeId` and
`vehicleNumber` **required**. Returns `201` with the new vehicle.

If the driver already has one, it returns:
...
with status `409`. Use the returned `vehicleId` to route the driver straight to
the edit screen.
...
| `POST` | `/vehicles` | Bearer | multipart (recovery only — 409 if one exists) |
...
4. **One vehicle per driver, forever.** `GET /vehicles/my` returns at most one
   entry; there is no add or delete flow after onboarding, only edit.
