...
Related: [Push Notifications](./push-notifications.md) ·
[User App Guide](./user-app-guide.md) ·
[Driver Auth & Onboarding](./driver-auth-onboarding.md)
...
Guard on a persisted flag so this runs exactly once:

```javascript
import AsyncStorage from '@react-native-async-storage/async-storage';
import { PlayInstallReferrer } from 'react-native-play-install-referrer';

const KEY = 'install_referrer_sent';

export const reportInstallReferrer = async () => {
  if (await AsyncStorage.getItem(KEY)) return;

  PlayInstallReferrer.getInstallReferrerInfo(async (info, err) => {
    if (err || !info?.installReferrer) return; // no flag set — try again next launch

    try {
      const res = await fetch(`${BASE_URL}/api/v3/install-referrers`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          referrer: info.installReferrer,
          install_time: info.installBeginTimestampSeconds,
          referrer_click_time: info.referrerClickTimestampSeconds,
          device_id: DEVICE_ID,      // same id you send to /link
          app: 'user',               // 'driver' in the driver app
          platform: 'android',
          app_version: VERSION,
        }),
      });

      // 200 and 201 are both "done". Only a network error or a 5xx is worth retrying.
      if (res.ok) await AsyncStorage.setItem(KEY, '1');
    } catch {
      // leave the flag unset; the next launch retries
    }
  });
};
```
